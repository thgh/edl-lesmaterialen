...
export default async function CourseMaterialPage({
  ...
  const title =
    (locale === 'de' ? material.title_de : material.title_nl) || material.title_de || material.title_nl ||
    material.title ||
    'Untitled'

  const firstAttachment = Array.isArray(material.attachments) && material.attachments.length > 0
  ...
  const firstAttachmentMime = firstAttachment && typeof firstAttachment !== 'string'
    ? (firstAttachment as any).mimeType || null
    : null

  const hasImageHero = !!(firstAttachmentId && firstAttachmentMime && firstAttachmentMime.startsWith('image/'))

  const externalLinks = material.links || []
  const hasExternalOnly = externalLinks.length > 0 && (!material.attachments || material.attachments.length === 0)
    ...
    <main className="px-4 py-6 sm:px-6 lg:px-8">
      ...
      <article className="mx-auto max-w-5xl">
          ...
          {hasExternalOnly ? (
            <div className="flex flex-wrap items-center gap-3">
              {externalLinks.map((lnk) => {
                const label = (locale === 'de' ? lnk.label_de : lnk.label_nl) || lnk.label_nl || lnk.label_de
                const url = lnk.url || '#'
                return (
                  <a
                    key={lnk.id || url}
            ...
            <div className="flex flex-wrap items-center gap-3">
              {Array.isArray(material.attachments) &&
                material.attachments.map((att) => {
                  ...
                  const filename = typeof att === 'string' ? undefined : ((att as any).filename as string | undefined)
        ...
        {externalLinks.length > 0 && !hasExternalOnly && (
          <section className="mb-10">
            <h2 className="mb-3 text-lg font-semibold text-gray-900">{dict.detailExternalLinksTitle}</h2>
            <ul className="list-inside list-disc space-y-2">
              {externalLinks.map((lnk) => {
                const label = (locale === 'de' ? lnk.label_de : lnk.label_nl) || lnk.label_nl || lnk.label_de || lnk.url
                return (
                  <li key={lnk.id || lnk.url}>
                    <a href={lnk.url || '#'} target="_blank" rel="noopener noreferrer" className="text-blue-700 hover:underline">
                      {label}
                    </a>
                  </li>
                )
        ...
        <section className="mb-8 grid gap-4 md:grid-cols-2">
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {material.materialTypes.map((t) => (
                  <span key={typeof t === 'string' ? t : (t as any).id} className="rounded bg-gray-100 px-2 py-0.5 text-sm">
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {material.competences.map((c) => (
                  <span key={typeof c === 'string' ? c : (c as any).id} className="rounded bg-gray-100 px-2 py-0.5 text-sm">
          ...
          {material.description_nl || material.description_de ? (
            <div className="md:col-span-2">
              <h3 className="text-sm font-semibold text-gray-700">{locale === 'de' ? 'Beschreibung' : 'Beschrijving'}</h3>
              <p className="mt-2 whitespace-pre-line text-gray-800">
                {(locale === 'de' ? material.description_de : material.description_nl) || material.description_nl || material.description_de}
              </p>
            </div>
          ) : null}
        </section>
